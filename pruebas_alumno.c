...
void agregarElementosSimplesAlComienzo()
{
	...
	pa2m_afirmar(
		...
		"Agregar un elemento fuera del rango permitido (posicion mayor a la cantidad acual), agregar elemento retorna false");
	...
	pa2m_afirmar(lista_cantidad_elementos(lista) == 5,
		     "Hay 5 elementos en la lista");
	...
	for (size_t i = 0; i < 5; i++) {
		...
		pa2m_afirmar(
			elemento == &numero[4 - i],
			"Obtener en posicion %li, dirección obtenido: '%p' -> esperado: '%p'",
			i, elemento, &numero[4 - i]);
	}
...
void agregarElementosSimplesEnElMedio() // La prueba de NULL
{
	...
	int *lista_esperada[] = { &numeros[5], &numeros[0], &numeros[7],
				  &numeros[1], &numeros[6], &numeros[9],
...
void agregarElemetosConMalloc()
{
	...
	for (size_t i = 0; i < 4; i++) {
		char *nombre = malloc(strlen(nombres[i]) + 1 * sizeof(char *));
		if (!nombre) {
	...
	for (size_t i = 0; i < 4; i++) {
		...
		pa2m_afirmar(
			...
			"Obtener en posicion %li, dirección obtenido: '%p' -> esperado: '%p'",
			i, elemento, direccioes_creadas_con_malloc[i]);
	}
...
void buscarElementoFuncionComparacion()
{
	...
	for (int i = 0; i < 10; i++) {
		buscar = numeros[i];
		encontrado = (int *)lista_buscar_elemento(
			lista, (void *)&buscar, comparar_numeros);
		pa2m_afirmar(
			encontrado == &numeros[i],
			"Se busca el elemento '%d', direccion obtenido: '%p' -> esperado: %p",
			buscar, encontrado, &numeros[i]);
	}

	buscar = 100;
	encontrado =
		lista_buscar_elemento(lista, (void *)&buscar, comparar_numeros);
	pa2m_afirmar(
		encontrado == NULL,
		"Buscar un elemento que no está en la lista, la función retornará NULL");
...
void eliminarYComprobarBuscando()
{
	...
	pa2m_afirmar(lista_cantidad_elementos(lista) == 8,
		     "Se agregaron 8 elementos diferentes en la lista");
	char *elemento_quitado;
	pa2m_afirmar(lista_quitar_elemento(lista, 0,
					   (void **)&elemento_quitado) == true,
		     "Se quitó el elemento en la posicion 0");
	pa2m_afirmar(
		elemento_quitado == &letras[0],
		"El elemento quitado por el inicio es '%c', direccion obtenida: '%p' -> esperado: '%p'",
		letras[0], elemento_quitado, &letras[0]);
	pa2m_afirmar(
		lista_buscar_elemento(lista, (void *)&letras[0],
				      comparar_letras) == &letras[7],
		"Buscar el elemento eliminado sí lo encuentra, hay repetido");
	pa2m_afirmar(lista_cantidad_elementos(lista) == 7,
		     "Hay 7 elementos en la lista");

	pa2m_afirmar(lista_quitar_elemento(lista,
					   lista_cantidad_elementos(lista) - 1,
					   (void **)&elemento_quitado) == true,
		     "Se quitó el elemento al final de la lista");
	pa2m_afirmar(
		elemento_quitado == &letras[7],
		"El elemento quitado por el final es '%c', direccion obtenida: '%p' -> esperado: '%p'",
		letras[7], elemento_quitado, &letras[7]);
	pa2m_afirmar(
		lista_buscar_elemento(lista, (void *)&letras[7],
				      comparar_letras) == false,
		"Buscar el elemento eliminado no lo encuentra, era el elemento repetido");
	pa2m_afirmar(lista_cantidad_elementos(lista) == 6,
		     "Hay 6 elementos en la lista");

	pa2m_afirmar(
		lista_quitar_elemento(lista, 2, NULL) == true,
		"Se quitó un elemento en el medio de la lista, auque se haya pasado NULL como parametro");
	pa2m_afirmar(lista_buscar_elemento(lista, (void *)&letras[3],
					   comparar_letras) == false,
		     "El elemento eliminado no se encuentra en la lista");
	pa2m_afirmar(lista_cantidad_elementos(lista) == 5,
		     "Hay 5 elementos en la lista");
	...
	pa2m_afirmar(
		elemento_obtenido == &letras[1],
		"Obtener en posicion 0, direccion obtenida: '%p', -> esperado: '%p'",
		elemento_obtenido, &letras[1]);
	lista_obtener_elemento(lista, 1, (void **)&elemento_obtenido);
	pa2m_afirmar(
		elemento_obtenido == &letras[2],
		"Obtener en posicion 1, direccion obtenida: '%p', -> esperado: '%p'",
		elemento_obtenido, &letras[2]);
	lista_obtener_elemento(lista, 2, (void **)&elemento_obtenido);
	pa2m_afirmar(
		elemento_obtenido == &letras[4],
		"Obtener en posicion 2, direccion obtenida: '%p', -> esperado: '%p'",
		elemento_obtenido, &letras[4]);
	lista_obtener_elemento(lista, 3, (void **)&elemento_obtenido);
	pa2m_afirmar(
		elemento_obtenido == &letras[5],
		"Obtener en posicion 3, direccion obtenida: '%p', -> esperado: '%p'",
		elemento_obtenido, &letras[5]);
	lista_obtener_elemento(lista, 4, (void **)&elemento_obtenido);
	pa2m_afirmar(
		elemento_obtenido == &letras[6],
		"Obtener en posicion 4, direccion obtenida: '%p', -> esperado: '%p'",
		elemento_obtenido, &letras[6]);
...
void eliminarPorElFinal()
{
	...
	for (size_t i = 0; i < 7; i++) {
		lista_quitar_elemento(lista,
				      lista_cantidad_elementos(lista) - 1,
				      (void **)&elemento_quitado);
		pa2m_afirmar(
			lista_cantidad_elementos(lista) == 6 - i,
			"Se quitó un elemento por el final, ahora quedan %li",
			6 - i);
		pa2m_afirmar(lista_buscar_elemento(lista, &numeros[6 - i],
						   comparar_numeros) == NULL,
...
void eliminarPorElInicio()
{
	...
	for (size_t i = 0; i < 7; i++) {
		...
		pa2m_afirmar(
			...
			"Se quitó un elemento por el final, ahora quedan %li",
			6 - i);
		pa2m_afirmar(lista_buscar_elemento(lista, &numeros[i],
						   comparar_numeros) == NULL,
			     "El elemento '%d' no se encuentra en la lista",
			     numeros[i]);
	}
...
void eliminarPorElMedio()
{
	...
	int numeros[] = { 1, -2, 5, -3, 8, 10, -4 };
	for (size_t i = 0; i < 7; i++) {
		if (!lista_agregar_elemento(lista, i, (void *)&numeros[i])) {
			lista_destruir_todo(lista, NULL);
			return;
		}
	}
	...
	for (size_t i = 0; i < 3; i++) {
		lista_quitar_elemento(lista, 2, (void **)&elemento_quitado);
		pa2m_afirmar(
			lista_cantidad_elementos(lista) == 6 - i,
			"Se quitó un elemento en la posicion 2, ahora hay %li elementos",
			6 - i);
		pa2m_afirmar(lista_buscar_elemento(lista, &numeros[i + 2],
						   comparar_numeros) == NULL,
			     "El elemento '%d' no se encuentra en la lista",
			     numeros[i + 2]);
	}
	...
	pa2m_afirmar(lista_cantidad_elementos(lista) == 4,
		     "Quedan 4 elementos en la lista");

	for (size_t i = 0; i < 2; i++) {
		if (i == 0) {
			lista_quitar_elemento(lista, 1,
					      (void **)&elemento_quitado);
			pa2m_afirmar(
				lista_cantidad_elementos(lista) == 3,
				"Se quitó un elemento en la posicion 1, ahora hay 3 elementos");
			pa2m_afirmar(
				lista_buscar_elemento(lista, &numeros[1],
						      comparar_numeros) == NULL,
				"El elemento '%d' no se encuentra en la lista",
				numeros[1]);
		} else {
			...
			pa2m_afirmar(
				...
				"Se quitó un elemento en la posicion 1, ahora hay 2 elementos");
			pa2m_afirmar(
				lista_buscar_elemento(lista, &numeros[5],
						      comparar_numeros) == NULL,
				"El elemento '%d' no se encuentra en la lista",
				numeros[5]);
		}
	...
	pa2m_afirmar(lista_cantidad_elementos(lista) == 2,
		     "Quedan 2 elementos en la lista");
	lista_obtener_elemento(lista, 0, (void **)&elemento_obtenido);
	pa2m_afirmar(
		elemento_obtenido == &numeros[0],
		"El elemento obtenido en la posicion 0: dirección obtenida: '%p' -> esperado '%p'",
		elemento_obtenido, &numeros[0]);
	lista_obtener_elemento(lista, 1, (void **)&elemento_obtenido);
	pa2m_afirmar(
		elemento_obtenido == &numeros[6],
		"El elemento obtenido en la posicion 1: dirección obtenida: '%p' -> esperado '%p'",
		elemento_obtenido, &numeros[6]);
	...
	pa2m_afirmar(
		...
		"Se quitaron los 2 últimos elementos, la lista se encuentra vacia");
...
void iterarTodosLosElementos()
{
	...
	size_t cantidad_iteraciones =
		lista_iterar_elementos(lista, sumar_elementos, &suma_total);
	pa2m_afirmar(
		cantidad_iteraciones == lista_cantidad_elementos(lista),
		"Si se itera todos los elementos, da como resultado la cantidad de elementos en la lista");
	pa2m_afirmar(suma_total == 55,
		     "La suma de todos los numeros iterados es 55");
	lista_destruir(lista);
}
...
void verificarCorte()
{
	...
	int multiplo_de_5 = 0;
	;
	size_t cantidad_iterados = lista_iterar_elementos(
		lista, buscar_el_multiplo, &multiplo_de_5);
	pa2m_afirmar(cantidad_iterados == 8,
		     "Cortó en la posición 7 de la lista");
	lista_obtener_elemento(lista, cantidad_iterados - 1,
			       (void *)&elemento_obtenido);
	pa2m_afirmar(
		*elemento_obtenido == multiplo_de_5,
		"El elemento obtenido es el mismo que el elemento que verificó el cierre del iterador");
	lista_destruir(lista);
}
...
void listaNula()
{
	...
	int multiplo_de_2;
	int buscado = 5;
	pa2m_afirmar(lista_buscar_elemento(lista, (void *)&buscado,
					   comparar_numeros) == NULL,
		     "Tener una lista nula, buscar elemento devuelve NULL");
	pa2m_afirmar(lista_obtener_elemento(lista, 0, NULL) == false,
		     "Tener una lista nula, obtener elemento devolverá false");
	pa2m_afirmar(lista_cantidad_elementos(lista) == 0,
		     "Lista nula devuelve 0 elementos");
	pa2m_afirmar(lista_quitar_elemento(lista, 0, NULL) == false,
		     "Lista nula, quitar elemento dará false");
	pa2m_afirmar(lista_iterar_elementos(lista, buscar_el_multiplo,
					    &multiplo_de_2) == 0,
		     "Tener una lista nula, el iterador interno devolverá 0");
}
...
void inicializarIteradorExternoConListaInicializada()
{
	...
	pa2m_afirmar(lista_cantidad_elementos(lista) == 0,
		     "La lista tiene 0 elementos, está vacía");
	pa2m_afirmar(
		lista_iterador_obtener_elemento_actual(iterador) == NULL,
		"No hay elementos en la lista, ver elemento actual retorna NULL");
	pa2m_afirmar(
		lista_iterador_hay_siguiente(iterador) == false,
		"Lista vacía, hay siguiente retorna false, tampoco avanza");
...
void iteradorExternoConElementosEnLista()
{
	...
	for (size_t i = 0; i < 4; i++) {
		nombre_obtenido =
			(char *)lista_iterador_obtener_elemento_actual(
				iterador);
		pa2m_afirmar(
			nombre_obtenido == nombres[i],
			"El elemeneto en la posicion %li, la dirección es: '%p', debe ser: '%p'",
			i, nombre_obtenido, nombres[i]);
		pa2m_afirmar(lista_iterador_hay_siguiente(iterador) == true,
			     "Hay siguiente elemento, avanza");
		lista_iterador_avanzar(iterador);
	}
	nombre_obtenido =
		(char *)lista_iterador_obtener_elemento_actual(iterador);
	pa2m_afirmar(lista_iterador_hay_siguiente(iterador) == false,
		     "Ya no hay elemento siguiente");
	pa2m_afirmar(
		nombre_obtenido == NULL,
		"Acabó de iterar todos los elementos de la lista, obtener elemento retorna NULL");
	lista_iterador_destruir(iterador);
	lista_destruir(lista);
}
...
void iteradorExternoNulo()
{
	...
	pa2m_afirmar(iterador == NULL,
		     "Pasar una lista nula, retorna NULL el iterador externo");
	pa2m_afirmar(lista_iterador_hay_siguiente(iterador) == false,
		     "Tener iterador nulo, hay siguiente retorna false");
	pa2m_afirmar(
		lista_iterador_obtener_elemento_actual(iterador) == NULL,
		"Tener iterador nulo, obtener elemento actual retorna NULL");
}
...
void apilarYDesapilarElementos()
{
	...
	pa2m_afirmar(pila_tope(pila) == NULL,
		     "No hay elementos, ver tope devuelve NULL");
	pila_destruir(pila);
}
...
void apilarDesapilarVariasVeces()
{
	...
	for (size_t i = 0; i < 2; i++) {
		...
		pa2m_afirmar(
			puntero == &decimales1[5 - i],
			"Elemento desapilado tiene la dirección '%p' y debe ser '%p'",
			puntero, &decimales1[5 - i]);
	}
	...
	pa2m_afirmar(
		pila_cantidad(pila) == 9,
		"Se agregaron 5 elementos más, ahora hay 9 elementos en la Pila");

	for (size_t i = 0; i < 5; i++) {
		puntero = (double *)pila_tope(pila);
		pa2m_afirmar(
			puntero == &decimales2[4 - i],
			"Ver el tope de la pila tiene la dirección '%p' y debe ser '%p'",
			puntero, &decimales2[4 - i]);
		puntero = (double *)pila_desapilar(pila);
		pa2m_afirmar(
...
void pilaNula()
{
	char *nombre = "Juan";
	void *pila = NULL;
	pa2m_afirmar(pila_apilar(pila, (void *)&nombre) == false,
		     "Tener una pila nula, apilar da false");
	pa2m_afirmar(pila_desapilar(pila) == NULL,
		     "Tener una pila nula, desapilar retorna NULL");
	pa2m_afirmar(pila_cantidad(pila) == 0,
		     "Tener una pila nula, la cantidad de elementos es 0");
	pa2m_afirmar(pila_esta_vacía(pila) == false,
		     "Tener una pila nula, está vacio retorna false");
	pa2m_afirmar(pila_tope(pila) == NULL,
		     "Tener una pila nula, ver el tope retorna NULL");
}
...
void inicializandoUnaCola()
{
	...
	pa2m_afirmar(
		...
		"Al no tener elementos en la cola, ver el frente retorna NULL");
...
void encolarYDesencolarElementos()
{
	...
	pa2m_afirmar(cola_esta_vacía(cola) == false,
	...
	for (size_t i = 0; i < 6; i++) {
		...
		pa2m_afirmar(
			puntero == &numeros[i],
			"Ver el frente de la cola tiene la dirección '%p' y debe ser '%p'",
			puntero, &numeros[i]);
		puntero = (int *)cola_desencolar(cola);
		pa2m_afirmar(
			puntero == &numeros[i],
			"Elemento desencolado tiene la misma direccion que el frente recién visto");
	}

	pa2m_afirmar(cola_cantidad(cola) == 0, "La cola se encuentra vacía");
	pa2m_afirmar(cola_frente(cola) == NULL,
		     "No hay elementos, ver frente devuelve NULL");
...
void encolarDesencolarVariasVeces()
{
	...
	for (size_t i = 0; i < 5; i++) {
		...
		pa2m_afirmar(
			elemento_desencolado == &decimales1[i],
			"El elemento desencolado tiene dirección '%p' y debe ser '%p'",
			elemento_desencolado, &decimales1[i]);
	}
	...
	pa2m_afirmar(
		cola_cantidad(cola) == 6,
		"Se agregaron 5 elementos más a la cola, ahora hay 6 elementos");

	for (size_t i = 0; i < 6; i++) {
		elemento_desencolado = (double *)cola_desencolar(cola);
		if (i < 1)
			pa2m_afirmar(
				elemento_desencolado == &decimales1[5],
				"El elemento desencolado tiene dirección '%p', y debe ser '%p'",
				elemento_desencolado, &decimales1[5]);
		else
			pa2m_afirmar(
				elemento_desencolado == &decimales2[i - 1],
				"El elemento desencolado tiene dirección '%p', y debe ser '%p'",
				elemento_desencolado, &decimales2[i - 1]);
	}
	pa2m_afirmar(cola_cantidad(cola) == 0, "Cola vacía");
	pa2m_afirmar(cola_frente(cola) == NULL,
		     "No hay elementos en la cola, ver el frente retorna NULL");
...
void colaNula()
{
	char *nombre = "Ernesto";
	Cola *cola = NULL;
	pa2m_afirmar(cola_encolar(cola, (void *)&nombre) == false,
		     "Tener una cola nula, encolar da false");
	pa2m_afirmar(cola_desencolar(cola) == NULL,
		     "Tener una cola nula, desencolar retorna NULL");
	pa2m_afirmar(cola_cantidad(cola) == 0,
		     "Tener una cola nula, la cantidad de elemenos es 0");
	pa2m_afirmar(cola_esta_vacía(cola) == false,
		     "Tener una cola nula, está vacio retorna false");
	pa2m_afirmar(cola_frente(cola) == NULL,
		     "Tener una cola nula, ver el frente retorna NULL");
}
...
int main()
{
	...
	pa2m_nuevo_grupo(
		"============= PREUBAS ITERADOR EXTERNO LISTA =============");
	...
	pa2m_nuevo_grupo("Prueba iterador nulo");
	iteradorExternoNulo();
